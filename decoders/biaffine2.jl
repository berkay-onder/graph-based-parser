...
function parsed(this::BiaffineDecoder2, parser, s_arcs_val, cache=nothing)
    ...
    for i = 1:size(s_arcs_val, 2)
        ...
        inds = inds .+ 1
        #if VERBOSE
        remove_invalids!(inds, T)
        #end
        root = indmax(sent[:, 1])
        res[i, :] = Int.([root, inds...])
        cache!=nothing && push!(cache, inds.-1)
    end
    return res
end

function remove_invalids!(inds, T)
    
    iset = Set(inds)
    stuff = find(x->x==1, inds)
    if isempty(stuff)
        stuff=[findfirst(x->(!(1<=x<=T)), inds)]
        inds[stuff[1]]=1
    end
    rootfound=rootfound0=false
    for j = 1:length(inds)
        if inds[j]==1
            rootfound0=true
        end
        if (!rootfound && !(1 <= inds[j] <= T)) || (rootfound && !(2 <= inds[j] <= T))
            VERBOSE && warn("Invalid arc ", inds[j], " not in 1..$T")
            inds[j] = stuff[1]+1
        end
        rootfound=rootfound0
    end
end
